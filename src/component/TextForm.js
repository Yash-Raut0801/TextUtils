...
export default function TextForm(props) {
  ...
  const handleCopyClick = () => {
    ...
    // text.select();
    navigator.clipboard.writeText(text); //if you wanna use above two line then write text.value inside the box
  ...
  return (
    <>
      <div className="contaienr" style={{color: props.mode==='light'?'black':'white'}}>
        ...
        <button disabled={text.length===0} className="btn btn-primary mx-1 my-1" onClick={handleTitleClick}>Convert to TitleCase(<i>Only first letter is capitalazied</i>)</button>
        <button disabled={text.length===0} className="btn btn-primary mx-1 my-1" onClick={handleTurkUpClick}>Convert to TurkishUpperCase</button>
      ...
      <div className="container my-3" style={{color: props.mode === 'light' ? 'black':'white' }}>
        ...
        <p><i>{text.split(/\s+/).filter((el)=>{return el.length!==0}).length} words, {text.length} characters</i></p>
